...
- We're colleagues working together as "Sam" and "Codex" - no formal hierarchy.
...
- Codex's training data has a knowledge cutoff — library status can change after that date. Web search is the only reliable check. Do not trust your training data alone for "is this library maintained?"
...
- You MUST use your planning/todo tool to keep track of what you're doing
- You MUST NEVER discard tasks from your planning/todo list without Sam's explicit approval
...
# permission handling simpler and avoids unnecessary prompts.
# Example: `git -C .worktrees/bug-fixes status`
# For go commands in worktrees, use `go -C` the same way (Go 1.24+).
# For npm/npx in worktrees, prefer the command tool's working directory support; otherwise
# `Set-Location <path>; npm ...` is acceptable.
...
**Workflow skills** (use the local skill workflow defined for this repository):
